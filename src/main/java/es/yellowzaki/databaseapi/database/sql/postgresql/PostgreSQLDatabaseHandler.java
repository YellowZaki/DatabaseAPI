...
public class PostgreSQLDatabaseHandler<T> extends SQLDatabaseHandler<T> {
     ...
     */
    PostgreSQLDatabaseHandler(JavaPlugin plugin, Class<T> type, DatabaseConnector databaseConnector, DatabaseSettings settings) {
    ...
    /* (non-Javadoc)
     * @see es.yellowzaki.databaseapi.database.sql.SQLDatabaseHandler#saveObject(java.lang.Object)
     */
    @Override
    public CompletableFuture<Boolean> saveObject(T instance) {
        ...
        // Null check
        if (instance == null) {
            DatabaseLogger.logError(plugin, "PostgreSQL database request to store a null. ");
