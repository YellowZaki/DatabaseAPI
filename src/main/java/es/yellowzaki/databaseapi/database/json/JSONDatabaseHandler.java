...
public class JSONDatabaseHandler<T> extends AbstractJSONDatabaseHandler<T> {
     ...
     */
    JSONDatabaseHandler(JavaPlugin plugin, Class<T> type, DatabaseConnector databaseConnector, DatabaseSettings settings) {
        ...
        for (File file: Objects.requireNonNull(tableFolder.listFiles((dir, name) ->  name.toLowerCase(Locale.ENGLISH).endsWith(JSON)))) {
            try (FileReader reader = new FileReader(file)){
                ...
                if (object != null) {
                    ...
                    DatabaseLogger.logError(plugin, "JSON file created a null object: " + file.getPath());
                    reader.close(); // NOSONAR Required to keep OS file handlers low and not rely on GC
                }
            } catch (FileNotFoundException e) {
                DatabaseLogger.logError(plugin, "Could not load file '" + file.getName() + "': File not found.");

            } catch (Exception e) {
                DatabaseLogger.logError(plugin, "Could not load objects " + file.getName() + " " + e.getMessage());
            }
        }
        return list;
    }
    ...
    public T loadObject(@NonNull String uniqueId) {
        ...
        try (FileReader reader = new FileReader(new File(plugin.getDataFolder(), fileName))) {
            ...
            DatabaseLogger.logError(plugin, "Could not load file '" + fileName + "': File not found.");
        } catch (Exception e) {
            DatabaseLogger.logError(plugin, "Could not load objects " + fileName + " " + e.getMessage());
        }
    ...
    public CompletableFuture<Boolean> saveObject(T instance) throws IntrospectionException, IllegalAccessException, InvocationTargetException {
        ...
        // Null check
        if (instance == null) {
            DatabaseLogger.logError(plugin, "JSON database request to store a null. ");
    ...
    private void store(CompletableFuture<Boolean> completableFuture, String toStore, File file, File tableFolder, String fileName) {
        try (FileWriter fileWriter = new FileWriter(file)) {
            ...
            DatabaseLogger.logError(plugin, "Could not save JSON file: " + tableFolder.getName() + " " + fileName + " " + e.getMessage());
            completableFuture.complete(false);
        }
    }
    ...
    private void delete(String uniqueId) {
        ...
        if (tableFolder.exists()) {
            ...
            try {
                ...
                DatabaseLogger.logError(plugin, "Could not delete JSON database object! " + file.getName() + " - " + e.getMessage());
            }
        }
    }
    ...
    public void deleteObject(T instance) {
        // Null check
        if (instance == null) {
            DatabaseLogger.logError(plugin, "JSON database request to delete a null.");
            return;
        }
        if (!(instance instanceof DataObject)) {
            DatabaseLogger.logError(plugin, "This class is not a DataObject: " + instance.getClass().getName());
